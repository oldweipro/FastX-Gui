...
class BackgroundManager(QObject):
    ...
    def _apply_efficient_blur(self, pixmap: QPixmap, blur_radius: int) -> QPixmap:
        ...
        try:
            ...
            if blur_radius > 20:
                ...
                small_size = QSize(
                    int(original_size.width() * 0.25),
                    int(original_size.height() * 0.25),
                )
    ...
    def _simple_blur(self, pixmap: QPixmap, radius: int) -> QPixmap:
        ...
        try:
            ...
            small_pixmap = pixmap.scaled(blurred_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)

            # Scale back up to original size for blur effect
            blurred_pixmap = small_pixmap.scaled(original_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
